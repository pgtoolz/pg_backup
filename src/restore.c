...
/*
 * Entry point of pg_backup RESTORE and VALIDATE subcommands.
 */
int
								   ...
								   pgRestoreParams *params, pgRecoveryTarget *rt)
{
	...
	fio_fprintf(fp, "# recovery.conf generated by %s %s\n",
...

{
	...
	if (params)
	{
		...
		fio_fprintf(fp, "\n# recovery settings added by %s restore of backup %s at '%s'\n",
				PROGRAM_NAME, backup_id_of(backup), current_time_str);
